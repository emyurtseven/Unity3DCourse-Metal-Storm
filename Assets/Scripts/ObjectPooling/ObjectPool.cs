...
/// * An ObjectPoolCapacitySO scriptable object asset named "ObjectPoolCapacityData" must be in "Resources" folder
/// 
...
public static class ObjectPool
{
    ...
    private static void InitializeDictionaries()
    {
        ...
        if (poolCapacityData == null)
        {
            Debug.LogWarning($"ObjectPoolOptionsSO scriptable object is missing in resources folder. " +
                            "Creating asset with default values. Make sure to create an asset with custom values");
            CreateDefaultAsset();
        }
    ...
    public static void ReturnAllPooledObjects()
    {
        foreach (Transform container in objectPoolsHolder.transform)
        {
            ...
            {
                if (Enum.TryParse(item.name, out PooledObjectType type))
                {
